...
export function getComments() {
  const fetchPromise = fetch(
    "https://wedev-api.sky.pro/api/v2/eldar/comments",
    {
      method: "GET",
      headers: {
        Authorization: token,
...
addFormButton.addEventListener("click", (event) => {
  ...
  if (inputName.value === "" || inputText.value.length < 1) {
    ...
    fetch("https://wedev-api.sky.pro/api/v2/eldar/comments", {
      method: "POST",
      body: JSON.stringify({
      ...
      .catch((error) => {
        if (error.message === "Сервер упал, подержите пожалуйста") {
          ...
          addCommentTitle.classList.add("hidden");
        } else if (error.message === "авторизация") {
          alert("Надо бы вам авторизоваться");
        } else {
